...
float4 PSPreview( float2 Texcoord: TEXCOORD0) : COLOR0
{
	...
	//return float4(pos/(MAX_DS*50),0,1);
...
PS_OUTPUT_N PSInitNormal( float2 Texcoord: TEXCOORD0)
{
	PS_OUTPUT_N Output;
	Output.Normal = float4(0,1,0,0);
	Output.BiNormal = float4(1,0,0,0);
	return Output;
}


PS_OUTPUT_N PSComputeNormal( float2 Texcoord: TEXCOORD0)
{
	PS_OUTPUT_N Output;
	...
	/*	
	// requiere Ps 3_0 o ps 2.0 con mas de 64 instrucciones
	float4 Vel = tex2D( Velocidad, Texcoord );
	float alfa = tex2D( Posicion, Texcoord ).w;
	
	float3 dir = normalize(float3(Vel.x, 0, Vel.y));
	float3 grad = normalize(float3(-Vel.z, 1, -Vel.w));
	float3 tg = normalize(cross(grad, dir));
	dir = cross(tg, grad);
	
	// matriz de rotacion sobre el eje tg
	float ca = cos(-alfa);
	float sa = sin(-alfa);
	float x = tg.x;
	float y = tg.y;
	...
	float3x3 rot = {ca+x*x*(1-ca),		a1-z*sa,		a2+y*sa,
					a1+z*sa,	ca+y*y*(1-ca),			a3-x*sa,
					a2-y*sa,	a3+x*sa,		ca+z*z*(1-ca)		};
	Output.Normal = float4(mul(tex2D(Normal,Texcoord),rot),0);
	Output.BiNormal = float4(mul(tex2D(BiNormal,Texcoord),rot),0);
	return Output;
	*/
	
}

// Idem mapa de normales
technique ComputeNormal
{
    pass P0
    {          
        PixelShader  = compile ps_3_0 PSInitNormal(); 
    }
    pass P1
    {          
        PixelShader  = compile ps_3_0 PSComputeNormal(); 
    }
}
	...
	/*
	float map_step = 1.0/64.0;
	// simulo un bilinear sampling compatible con el heighmap.
	// El normal de la textura no es compatible porque toma los 4 texel mas cercanos
	// en cambio el height map esta armado diferente  (Se podria arreglar modificando el height map. )
	// Ojo: como estoy sampleando a manopla, tiene que tener puesto NONE en los filters
	float i = y/currentScaleXZ+32;
	float j = x/currentScaleXZ+32;
	float fracc_i = frac(i);
	float fracc_j = frac(j);
	i = (int)i;
	j = (int)j;
	i = i/64.0 + map_desf;
	j = j/64.0 + map_desf;
	
	float3 T0 = tex2Dlod(heightMap,float4(i,j,0,0));
	float3 T1 = tex2Dlod(heightMap,float4(i+map_step,j,0,0));
	float3 T2 = tex2Dlod(heightMap,float4(i,j+map_step,0,0));
	float3 T3 = tex2Dlod(heightMap,float4(i+map_step,j+map_step,0,0));
	float H0 = T0.r*0.299 + T0.g*0.587 + T0.b*0.114;
	float H1 = T1.r*0.299 + T1.g*0.587 + T1.b*0.114;
	float H2 = T2.r*0.299 + T2.g*0.587 + T2.b*0.114;
	float H3 = T3.r*0.299 + T3.g*0.587 + T3.b*0.114;
	float H = (H0 * (1 - fracc_i) + H1 * fracc_i) * (1 - fracc_j) +
				(H2 * (1 - fracc_i) + H3 * fracc_i) * fracc_j;
	
	return 255*H*currentScaleY;
	*/
